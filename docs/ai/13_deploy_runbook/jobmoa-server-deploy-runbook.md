# 배포 런북 상세 (jobmoa-server Deploy Runbook)
...
* **단계:**
  ...
  4. `cp build/libs/jobmoa-server-0.0.1-SNAPSHOT.jar /opt/app/app.jar`
  5. `sudo systemctl restart app.service`

**주의:** 산출물 jar 이름(`jobmoa-server-0.0.1-SNAPSHOT.jar`)이 `build.gradle`의 `version`에 의존한다 — 버전을 바꾸면 workflow의 `cp` 경로도 함께 갱신해야 한다(현재 하드코딩).
