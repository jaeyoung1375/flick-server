# 테스트 전략 (flick-server Test Strategy)

## 1. 현재 상태 (2026-09-13 실측)

* `src/test/java/kr/co/flick/FlickServerApplicationTests.java` — Spring Context 로드 확인용 기본 생성 테스트.
* `auth/util/JwtTokenUtilTest.java`, `code/service/CodeServiceTest.java` — 인프라 레이어 단위 테스트. **스트리밍 도메인 로직 테스트는 아직 없음**(해당 도메인 자체가 미구현).
