...
- [ ] 다른 사용자의 리소스 ID를 받는 API라면 소유권(`USER_ID` 일치)을 서비스 레이어에서 검증했는가 (IDOR 방지)
- [ ] 관리자 전용 API(`/admin/**`)를 추가했다면, 현재 role 인가가 없다는 한계([`12_known_issues`](../12_known_issues/flick-server-known-issues.md))를 인지하고 있는가
...
- [ ] 새 의존성을 추가했다면 [`02_tech_stack`](../02_tech_stack/flick-server-tech-stack.md)에도 반영했는가

## 문서 동기화
- [ ] API 계약이 바뀌었다면 [`09_api_contract`](../09_api_contract/flick-server-api-contract.md)를 갱신했는가
- [ ] 테이블/컬럼이 바뀌었다면 [`10_data_model`](../10_data_model/flick-server-data-model.md)을 갱신했는가
- [ ] 변경 사항을 [`11_changelog`](../11_changelog/flick-server-changelog.md)에 남겼는가
- [ ] 새로 발견한 함정이 있다면 [`12_known_issues`](../12_known_issues/flick-server-known-issues.md)에 남겼는가