# 코딩 규칙 상세 (jobmoa-server Coding Standards)
...
* 관리자 전용 엔드포인트는 `/admin/...`, 공개(비로그인) 엔드포인트는 `/public/...` 경로 세그먼트를 쓴다(`/admin/exercises`, `/public/codes` 등). 별도 인가 로직은 없고 관례상 구분이다 — [`12_known_issues`](../12_known_issues/jobmoa-server-known-issues.md) 참고.
...
* 도메인별 에러코드는 `common/code/<Domain>ErrorCode.java`에 `ResponseCode` 구현 enum으로 둔다(코드 접두어: 공통 없음 `CommonErrorCode`, 사용자 `UserErrorCode`, 파일 `FileErrorCode`, 공통코드 `CodeErrorCode`(`C0xxx`), 운동마스터 `ExerciseErrorCode`(`EX0xxx`), 운동프로필 `FitnessErrorCode`(`FT0xxx`), 운동기록 `WorkoutErrorCode`(`WR0xxx`)). 새 도메인 추가 시 이 패턴을 따라 새 enum을 만든다.
...
* 부분 수정(PUT)은 `COALESCE(#{dto.field}, EXISTING_COLUMN)` 패턴으로 "요청에 없는 필드는 기존 값 유지"를 구현한다(`FitnessProfile-mapper.xml` 실측) — 새 부분수정 API도 이 패턴을 따른다.
